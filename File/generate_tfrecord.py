...
for i in fr.readlines():
    item = i.split()
    img = np.float64(misc.imread(root_path + imagefile +'train_images/' + item[0]))
    img = img - meanvalue
    maskmat = sio.loadmat(root_path + imagefile +'train_mats/' + item[1])
...
for string_record in record_iterator:
    ...
    # plt.subplot(1,2,1)
    # plt.imsave("img.png",np.uint8(reconstructed_img))
    # plt.imshow(np.uint8(reconstructed_img))
    # plt.subplot(1,2,2)
    plt.imsave(root_path + imagefile + "train_ano/" + name ,np.uint8(reconstructed_mask))
